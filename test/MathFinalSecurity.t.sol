...
contract ApexV2MathFinalSecurityTest is Test {
    ...
    function setUp() public {
        math = new ApexMathHarness();

        uq = new UQ112x112Harness();
    }
    ...
    function test_min_returnsXWhenXLower() public view {
        assertEq(math.min(1, 2), 1);
    }

    function test_min_returnsYWhenYLower() public view {
    ...
    function test_sqrt_perfectSquares() public view {
        ...
        assertEq(math.sqrt(1 ether), 1_000_000_000);
    }

    function test_sqrt_roundsDown() public view {
        ...
        assertEq(math.sqrt(24), 4);
    }

    function test_sqrt_maxUint() public view {
    ...
    function testFuzz_sqrt_lowerBound(uint256 x) public view {
        ...
        assertLe(z, type(uint128).max);
    }

    function testFuzz_sqrt_definition(uint128 raw) public view {
        uint256 x = uint256(raw);

        uint256 square = x * x;

        uint256 z = math.sqrt(square);

        assertEq(z, x);
    }

    function testFuzz_sqrt_roundingProperty(uint128 raw) public view {
        uint256 y = uint256(raw);

        uint256 z = math.sqrt(y);
        ...
        if (z < type(uint128).max) {
            ...
            assertGt(next * next, y);
        }
    }

    function testFuzz_sqrt_monotonic(uint128 a, uint128 b) public view {
        uint256 x = uint256(a);
        ...
        assertLe(math.sqrt(x), math.sqrt(y));
    }
    ...
    function test_encode_zero() public view {
        assertEq(uq.encode(0), 0);
    }

    function test_encode_one() public view {
        assertEq(uq.encode(1), Q112);
    }

    function test_encode_two() public view {
        assertEq(uq.encode(2), Q112 * 2);
    }

    function test_encode_maxUint112() public view {
        uint112 value = type(uint112).max;

        uint224 encoded = uq.encode(value);

        assertEq(encoded, uint224(value) * Q112);
    }

    function testFuzz_encode(uint112 value) public view {
        uint224 encoded = uq.encode(value);
    ...
    function testFuzz_encode_monotonic(uint112 a, uint112 b) public view {
        ...
        assertLe(uq.encode(a), uq.encode(b));
    }
    ...
    function test_uqdiv_basic() public view {
        uint224 encoded = uq.encode(10);

        uint224 result = uq.uqdiv(encoded, 2);

        assertEq(result, Q112 * 5);
    }

    function test_uqdiv_one() public view {
        uint224 encoded = uq.encode(123);

        assertEq(uq.uqdiv(encoded, 1), encoded);
    }

    function test_uqdiv_zeroNumerator() public view {
        assertEq(uq.uqdiv(0, 1), 0);
    }
    ...
    function test_uqdiv_maxValues() public view {
        ...
        assertEq(uq.uqdiv(x, y), x / uint224(y));
    }

    function testFuzz_uqdiv_matchesNativeDivision(uint224 x, uint112 y) public view {
        vm.assume(y != 0);

        uint224 result = uq.uqdiv(x, y);

        assertEq(result, x / uint224(y));
    }

    function testFuzz_uqdiv_resultNeverExceedsNumeratorForDivisorAboveOne(uint224 x, uint112 y) public view {
        vm.assume(y > 1);
    ...
    function test_encodeAndDivide_oneToOne() public view {
        uint224 encoded = uq.encode(100);

        uint224 result = uq.uqdiv(encoded, 100);

        assertEq(result, Q112);
    }

    function test_encodeAndDivide_twoToOne() public view {
        uint224 encoded = uq.encode(200);
    ...
    function testFuzz_encodeAndDivide(uint112 numerator, uint112 denominator) public view {
        vm.assume(denominator != 0);

        uint224 encoded = uq.encode(numerator);

        uint224 result = uq.uqdiv(encoded, denominator);
