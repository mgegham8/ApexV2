...
contract ApexV2PairBurnSecurityTest is Test {
    ...
    function testBurnReturnsCorrectAmounts() public {
    ...
    function testBurnCannotStealMoreThanLiquidity() public {
        uint256 liquidity = pair.balanceOf(address(this));
        ...
        uint256 balance0After = token0.balanceOf(address(pair));

        uint256 balance1After = token1.balanceOf(address(pair));

        assertLt(balance0After, balance0Before);

        assertLt(balance1After, balance1Before);
    }

    function testBurnWithFeeOnMintsProtocolFee() public {
        ...
        uint256 feeLiquidity = pair.balanceOf(feeReceiver);

        assertGt(feeLiquidity, 0);
    }

    function testBurnZeroLiquidityFails() public {
        vm.expectRevert();
