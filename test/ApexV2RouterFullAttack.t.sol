...
function testDonationIsCountedAsSwapInput() public {
    ...
    assertEq(
        reserve0Before,
        1000 ether,
        "BAD_INITIAL_RESERVE0"
    );

    assertEq(
        reserve1Before,
        1000 ether,
        "BAD_INITIAL_RESERVE1"
    );
    ...
    (
        uint112 reserve0AfterDonation,
        uint112 reserve1AfterDonation,
        uint32 timestampAfterDonation
    ) = ApexV2Pair(pair).getReserves();

    timestampAfterDonation;

    assertEq(
        reserve0AfterDonation,
        1000 ether,
        "DONATION_CHANGED_RESERVE0"
    );

    assertEq(
        reserve1AfterDonation,
        1000 ether,
        "DONATION_CHANGED_RESERVE1"
    );
    ...
    uint256 actualBalance1 =
        token1.balanceOf(pair);

    assertEq(
        actualBalance0,
        1500 ether,
        "DONATION_NOT_IN_BALANCE"
    );
    ...
    assertEq(
        amount1Received,
        166249791562447890611,
        "UNEXPECTED_SWAP_OUTPUT"
    );
...
function testDonationThenSwapThenRemoveLiquidity() public {
    ...
    uint lpBefore = IERC20(pair).balanceOf(address(this));
...
function testDonationAttackAgainstExistingLP() public {
    ...
    uint aliceLP = IERC20(pair).balanceOf(alice);
    ...
    (uint aliceToken0, uint aliceToken1) =
        router.removeLiquidity(
            address(token0),
            address(token1),
            aliceLP,
...
function testDonationAttackEconomicProfit() public {
    ...
    uint256 token0Spent =
        bobToken0Before - bobToken0After;

    uint256 token1Gained =
        bobToken1After - bobToken1Before;
    ...
    uint256 bobInitialValue =
        bobToken0Before +
        bobToken1Before;

    uint256 bobFinalValue =
        bobToken0After +
        bobToken1After;
    ...
    if (bobFinalValue > bobInitialValue) {
        console.log(
            "RESULT: Bob appears economically profitable"
        );

        console.log(
            "Profit:",
            bobFinalValue - bobInitialValue
        );
    } else {
        console.log(
            "RESULT: Bob is NOT economically profitable"
        );

        console.log(
            "Loss:",
            bobInitialValue - bobFinalValue
        );
    }
...
function testDonationAttackEconomicProfitFuzz(
    ...
    uint256 token0Spent =
        initialToken0 - finalToken0;

    uint256 token1Gained =
        finalToken1 - initialToken1;
    ...
    assertLe(
        finalValue,
        initialValue,
        "Donation attack produced economic profit"
    );
