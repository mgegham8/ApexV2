...
contract ApexV2PairEdgeCaseSecurityTest is Test {
    ...
    function testSkimCannotStealReserves() public {
    ...
    function testSyncUpdatesDonationOnly() public {
    ...
    function testTinyLiquidityCannotMintFreeTokens() public {
        MockERC20 tinyToken0 = new MockERC20("Tiny0", "T0");

        MockERC20 tinyToken1 = new MockERC20("Tiny1", "T1");

        ApexV2Pair tinyPair = ApexV2Pair(factory.createPair(address(tinyToken0), address(tinyToken1)));

        tinyToken0.mint(address(this), 10);

        tinyToken1.mint(address(this), 10);

        tinyToken0.transfer(address(tinyPair), 1);

        tinyToken1.transfer(address(tinyPair), 1);
    ...
    function testRoundingCannotCreateLiquidity() public {
    ...
    function testReserveCannotExceedUint112() public {
    ...
    function testExtremeSwapFailsSafely() public {
        vm.expectRevert();

        pair.swap(type(uint112).max, 0, address(this), "");
    }
}
