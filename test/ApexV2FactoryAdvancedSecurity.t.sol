...
contract ApexV2FactoryAdvancedSecurityTest is Test {
    ...
    function testPairCreatedEvent() public {
        vm.recordLogs();
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].topics[0] == keccak256("PairCreated(address,address,address,uint256)")) {
                found = true;

                address createdPair;
                uint256 length;

                (createdPair, length) = abi.decode(logs[i].data, (address, uint256));

                assertEq(createdPair, pair);

                assertEq(length, 1);

                assertEq(address(uint160(uint256(logs[i].topics[1]))), address(tokenA));
    ...
    function testAllPairsLengthIncreases() public {
    ...
    function testDifferentPairsHaveDifferentAddresses() public {
    ...
    function testInitializeCannotBeCalledByAttacker() public {
        address pair = factory.createPair(address(tokenA), address(tokenB));
        ...
        p.initialize(address(tokenC), address(tokenA));
    }

    function testPairStoredInAllPairs() public {
        address pair = factory.createPair(address(tokenA), address(tokenB));

        assertEq(factory.allPairs(0), pair);
    }

    function testReverseLookupReturnsSamePair() public {
