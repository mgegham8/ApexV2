...
contract ApexV2PairHandler is Test {
    ...
    uint256 private constant MIN_ACTION_AMOUNT = 1e6;

    uint256 private constant MAX_ACTION_AMOUNT = 100 ether;

    constructor(ApexV2Pair _pair, MockERC20 _token0, MockERC20 _token1) {
        pair = _pair;
        token0 = _token0;
        token1 = _token1;
        ...
        bool success0 = token0.transfer(address(pair), INITIAL_LIQUIDITY);

        bool success1 = token1.transfer(address(pair), INITIAL_LIQUIDITY);

        require(success0 && success1, "INITIAL_TRANSFER_FAILED");
    ...
    function addLiquidity(uint256 seed) external {
        ...
        uint256 amount0 = bound(seed, MIN_ACTION_AMOUNT, MAX_ACTION_AMOUNT);
         ...
         * integer rounding.
         */
        uint256 amount1 = (amount0 * uint256(reserve1)) / uint256(reserve0);
         ...
         */
        if (pairBalance0 > type(uint112).max || pairBalance1 > type(uint112).max) {
            return;
        }

        if (amount0 > type(uint112).max - pairBalance0 || amount1 > type(uint112).max - pairBalance1) {
            return;
        }
        ...
        require(success0 && success1, "ADD_TRANSFER_FAILED");
    ...
    function removeLiquidity(uint256 seed) external {
        ...
        uint256 liquidity = bound(seed, 1, balance);

        bool transferred = pair.transfer(address(pair), liquidity);

        require(transferred, "LP_TRANSFER_FAILED");

        (uint256 amount0, uint256 amount1) = pair.burn(address(this));

        require(amount0 > 0 && amount1 > 0, "ZERO_BURN_OUTPUT");
    ...
    function swap(uint256 seed) external {
        (uint112 reserve0, uint112 reserve1,) = pair.getReserves();
         ...
         */
        if (reserve0 <= MIN_ACTION_AMOUNT * 5 || reserve1 == 0) {
            return;
        }

        uint256 maxAmountIn = uint256(reserve0) / 5;

        if (maxAmountIn < MIN_ACTION_AMOUNT) {
            return;
        }

        uint256 amount0In = bound(seed, MIN_ACTION_AMOUNT, maxAmountIn);
        ...
        bool transferred = token0.transfer(address(pair), amount0In);

        require(transferred, "SWAP_TRANSFER_FAILED");

        pair.swap(0, amount1Out, address(this), bytes(""));
    ...
    function donation(uint256 seed0, uint256 seed1) external {
        uint256 amount0 = bound(seed0, 1, MAX_ACTION_AMOUNT);

        uint256 amount1 = bound(seed1, 1, MAX_ACTION_AMOUNT);
        ...
        require(success0 && success1, "DONATION_TRANSFER_FAILED");
